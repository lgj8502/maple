...
void cUIMgr::InventoryClick()
{
	if (m_isInvenPicked == false)
	{
		...
		int a = (int)FindUI("인벤스크롤바")->m_Value;
		int value = a * 4;

		ID2D1Bitmap* bitmap = nullptr;

		if (FindUI("장비탭")->m_isOn == true)
		{
		...
		else if (FindUI("소비탭")->m_isOn == true)
		{
		...
		AddImage("아이템클릭", L"Hide", { 0,0 }, { 1.0f, 1.0f }, 0.5f);
		BitMapAdd("아이템클릭", bitmap);
		FindUI("아이템클릭")->m_Renderer.ChangeBitmap(1);

		auto itemClick = [](void) {
			POINT pos = UI_MGR->GetMousePoint();

			UI_MGR->FindUI("아이템클릭")->m_Transform.SetPos((float)pos.x, (float)pos.y);
		};

		AddEvent("아이템클릭", ADDEVENT_Update, itemClick);
...
{
	if (FindUI("인벤토리창1")->m_isActive == false) return;
...
void cUIMgr::InventoryRender(int _array[28])
{
	...
	FindUI("메소금액")->m_Text = to_string(PLAYER_MGR->m_player->m_InvenInfo.m_Meso);

	if (_array == nullptr) return;

	if (FindUI("소비탭")->m_isOn == true)
	{
...
{
	if (FindUI("장비창1")->m_isActive == false) return;
...
void cUIMgr::EquipRender()
{
	...
	m_EquipName.push_back("장비칸모자");
	m_EquipName.push_back("장비칸옷");
	m_EquipName.push_back("장비칸바지");
	m_EquipName.push_back("장비칸신발");
	m_EquipName.push_back("장비칸무기");
...
void cUIMgr::SkillClick()
{
	...
	AddEvent("아이템클릭", ADDEVENT_Update, SkillClick);
...
void cUIMgr::Render()
{
	...
	if (FindUI("소비탭") == nullptr || FindUI("인벤토리창1")->m_isActive == false) return;
